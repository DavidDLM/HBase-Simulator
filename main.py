...
def drop(table_name):
    ...
    if table_name+".json" in tables:
        if is_enabled(table_name):
            print(
                f"- {table_name} is still enable, please disable it before dropping")
        else:
    ...
    if row_name in data.keys():
        ...
        for col_fam, cols in data[row_name].items():
            for key, val in cols.items():
                if val[0]:
                    print(
                        f"{col_fam}:{key}\t\tTimestamp {val[-1][1]}, \t value={val[-1][0]}")
                else:
    ...
    while running:
        ...
        if command in "help?":
            ...
            try:
                ...
                print(
                    f"Invalid arguments, type '{command}?' for more information ")
        else:
